...
namespace Microsoft.Azure.Commands.Sql.ServerActiveDirectoryAdministrator.Services
{
    ...
    public class AzureSqlServerActiveDirectoryAdministratorAdapter
    {
        ...
        public ActiveDirectoryClient ActiveDirectoryClient
        {
            get
            {
                if (_activeDirectoryClient == null)
                {
                    ...
                    {
                        throw new ArgumentException(string.Format(Properties.Resources.InvalidGraphEndpoint));
                    }
                    _activeDirectoryClient = new ActiveDirectoryClient(Context);
                }
        ...
        public static AzureSqlServerActiveDirectoryAdministratorModel CreateServerActiveDirectoryAdministratorModelFromResponse(string resourceGroup, string serverName, ServerAdministrator admin)
        {
        ...
        protected ServerAdministratorCreateOrUpdateProperties GetActiveDirectoryInformation(string displayName, Guid objectId)
        {
            ...
            if (groupList.Count() > 1)
            {
                ...
                throw new ArgumentException(string.Format(Properties.Resources.ADGroupMoreThanOneFound, displayName));
            }
            if (groupList.Count() == 1)
            {
                ...
                {
                    throw new ArgumentException(string.Format(Properties.Resources.InvalidADGroupNotSecurity, displayName));
                }
            ...
            if (userList == null || userList.Count() == 0)
            {
                throw new ArgumentException(string.Format(Properties.Resources.ADObjectNotFound, displayName));
            }
            if (userList.Count() > 1)
            {
                // More than one user was found.
                throw new ArgumentException(string.Format(Properties.Resources.ADUserMoreThanOneFound, displayName));
            }
